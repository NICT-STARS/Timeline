...
/*-----* move *---------------------------------------------------------------*/
          var fncMove = function(pEvent)
          {
            try
            {
              ...
              var intMoveX = (flgEvent ? event.changedTouches[0].pageX : flgTouch ? pEvent.originalEvent.touches.item(0).pageX : pEvent.pageX);

              if ($rail.offset().left <= intMoveX && intMoveX <= $rail.offset().left + $rail.width())
              {
                _movePick(intMoveX);
              }
            }
...
/* create                                                                     */
/******************************************************************************/
  create : function(pTimeInfo)
  {
    var $main = $(".k2go-timeline-main");

    if (typeof pTimeInfo != "undefined")
    {
      _checkTimeInfo(pTimeInfo);

      $main.data("options.k2goTimeline").    minTime.setTime(pTimeInfo.    minTime.getTime());
      $main.data("options.k2goTimeline").    maxTime.setTime(pTimeInfo.    maxTime.getTime());
      $main.data("options.k2goTimeline").  startTime.setTime(pTimeInfo.  startTime.getTime());
      $main.data("options.k2goTimeline").    endTime.setTime(pTimeInfo.    endTime.getTime());
      $main.data("options.k2goTimeline").currentTime.setTime(pTimeInfo.currentTime.getTime());
    }
    else
...
function _createTimeline()
{
  try
  {
    ...
    if (typeof $main.data("options.k2goTimeline").timeChange == "function")
    {
      setTimeout(function()
      {
        $main.data("options.k2goTimeline").timeChange(
        {
          startTime   : new Date($main.data("options.k2goTimeline").  startTime.getTime()),
          endTime     : new Date($main.data("options.k2goTimeline").    endTime.getTime()),
          currentTime : new Date($main.data("options.k2goTimeline").currentTime.getTime())
  ...
  {
    console.error("jQuery.k2goTimeline _createTimeline error: " + pError);
  }
}
/******************************************************************************/
/* _incrementBar                                                              */
...
function _moveBar(pMoveX)
{
  try
  {
    ...
    objTimeInfo.currentTime = new Date($main.data("options.k2goTimeline").currentTime.getTime() - $main.data("options.k2goTimeline").scale * pMoveX);
