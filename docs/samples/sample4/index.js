...
$(window).on("load", function()
{
  for (var i01 = 0; i01 < $Env.data.length; i01++)
  {
    ...
    $("#legend").append("<li><span>" + $Env.data[i01].name + "</span><span style='background:" + $Env.data[i01].color + "'></span></li>");
  }
    ...
    timeChange       : function(pTimeInfo)
    {
      ...
      for (var i01 = 0; i01 < $Env.data.length; i01++)
      {
        ...
        ||  (pTimeInfo.startTime.getTime() >  objData.start.getTime() && objData.end  .getTime() >  pTimeInfo.  endTime.getTime()))
        {
          ...
          $("#data_bar").append($("<div></div>").css({ background : objData.color, left : intLeft + "px", width : intWidth + "px" }).attr("index", i01).toggleClass("selected", objData.selected));

          if (objData.start.getTime() <= pTimeInfo.currentTime.getTime() && pTimeInfo.currentTime.getTime() <= objData.end.getTime())
          {
            $Env.currentData = i01;
            $("#current_time"         ).html($timeline.k2goTimeline("formatDate", pTimeInfo.currentTime, "<span>" + objData.name + "</span><br/><span>%y(%jp)年%mm月%dd日%H時%M分%S秒</span>", 540)).css({ background : objData.color });
...
/* jp_calendar.click                                                          */
/******************************************************************************/
$("#jp_calendar").on("click", function()
{
  $(this       ).toggleClass("jp");
  $("#timeline").k2goTimeline("setOptions", { jpCalendar : $(this).hasClass("jp") });
  $("#timeline").k2goTimeline("create");
});
/******************************************************************************/
/* data_bar.click                                                             */
/******************************************************************************/
$("#data_bar").on("click", "> *", function()
{
  var $this    = $(this);
  var intIndex = parseInt($this.attr("index"), 10);
  var objData  = $Env.data[intIndex];

  objData.selected = !objData.selected;
  $this.toggleClass("selected", objData.selected);
});
/******************************************************************************/
/* prev or next.click                                                         */
/******************************************************************************/
